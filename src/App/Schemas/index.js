...
const getResolversInDirectory = directory => {
  ...
  return allResolvers.reduce((allResolversInDirectory, currentResolvers) => {
    for (let i in currentResolvers) {
      allResolversInDirectory[i] = {
        ...allResolversInDirectory[i],
        ...currentResolvers[i]
      };
    }
...
const typeDefs = getFoldersInDirectory(__dirname).map(folder => {
...
const resolvers = getFoldersInDirectory(__dirname)
  ...
  .map(filteredFolder =>
    getResolversInDirectory(
      path.resolve(__dirname, filteredFolder, 'Resolvers')
    )
  )
